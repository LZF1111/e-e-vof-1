...
/*            if (cAlpha.found())
		...
		scalar Q = cAlpha();
                if (Q==2)
                {
                    const volVectorField gradAlpha = phase2*fvc::grad(phase) - phase*fvc::grad(phase2);
                    const volScalarField gamma = mag(gradAlpha)/max(mag(gradAlpha));

                    forAll(gamma.internalField(),celli)
                    {
                      if (gamma[celli] > 0.4)
                          {
                           Q=1;
             ...
             Info<<"cAlpha()1 "<<cAlpha()<<endl; 
              Info<<"cAlpha()2 "<<Q<<endl;
              Info<<"gamma"<<gamma.internalField()<<endl;
	     ...
	     phir += min(Q*phic, max(phic))*nHatf(phase, phase2);*/
	     ...
	     volScalarField T
            ...
            mesh_,
            dimensionedScalar("T",dimensionSet(0, 0, 0, 0, 0, 0, 0), 0)
            );

            volVectorField gradAlpha1 = phase2*fvc::grad(phase) - phase*fvc::grad(phase2);
            volScalarField MagA = mag(gradAlpha1);
            volScalarField Cut = mag(gradAlpha1)/max(mag(gradAlpha1));
        //    float CUT = MagA.weightedAverage(mesh_.V()).value();

           // Info<< "Cut"<<Cut<<endl  ;
            Info<< "maxAlphagradient"<<max(mag(gradAlpha1))<<endl  ;
            Info<< "minAlphagradient"<<min(mag(gradAlpha1))<<endl  ;
	    //Info<< "weightAlphagradient"<<MagA.weightedAverage(mesh_.V()).value()<<endl  ;

            if (cAlpha.found())
            //if (cAlpha.found() && (K(phase1,phase2) <= 0.4))
            {
		    ...
		    forAll(T, i)
        	{
                       ...
                       // Info<< "V=" <<mesh_.V()[i]<<endl;

            		//if(mesh_.C()[i].y()>0.5) //(1)position base

            		//if(mesh_.V()[i] <= 3.89944e-06)// (2)Cell Volume  base

                       if(Cut[i] >= 0.4) //(3)alpha gradient base
                       //if(Cut <= MagA[i])//(4)mean alpha gradient base
                       T[i] = 1;
                       else
		...
		(
		    ...
		    mesh_,
		    dimensionedScalar("Q",dimensionSet(0, 0, 0, 0, 0, 0, 0), 0)
		);
		 Q += fvc::interpolate(T)    ;

		 //Info<< "Q  "<<Q<<endl;
		 phir += min(cAlpha()*phic*Q, max(phic))*nHatf(phase, phase2);
